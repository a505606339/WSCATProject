...
namespace HelperUtility.Excel
{
    public class NPOIExcelHelper
    {
        ...
        /// <param name="strHeaderText">表头文本</param>
        /// <param name="strFileName">保存位置</param>
        /// <param name="headerList">要生成的列头文本</param>
        public static void DataTableToExcel(DataTable dtSource, string strHeaderText, string strFileName, string[] headerList)
        {
            using (MemoryStream ms = DataTableToExcel(dtSource, strHeaderText,headerList))
            {
                using (FileStream fs = new FileStream(strFileName, FileMode.Create, FileAccess.Write))
                {
                    try
        ...
        /// <param name="dtSource">源DataTable</param>
        /// <param name="strHeaderText">表头文本</param>
        public static MemoryStream DataTableToExcel(DataTable dtSource, string strHeaderText, string[] columnHeader)
        {
            HSSFWorkbook workbook = new HSSFWorkbook();
            HSSFSheet sheet = (HSSFSheet)workbook.CreateSheet();
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                if (rowIndex == 65535 || rowIndex == 0)
                {
                    ...
                    #region 列头及样式
                    {
                        ...
                        {
                            headerRow.CreateCell(i).SetCellValue(columnHeader[i]);
